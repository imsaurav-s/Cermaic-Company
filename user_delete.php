...
<body style="background-size:cover" background="images/7.jpg">
	<form action="user_delete.php" method="POST">
		<div>Enter USER_ID:<input type="text" name="USER_ID"><br></div>
		<br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if($a!="")
        {
            $sql1 = "SELECT * from user where user_id='$a'";
            $result = mysqli_query($con,$sql1);

            if(mysqli_num_rows($result)>0)
            {
            $sql3="DELETE from user where user_id='$a'";  
