...
<body style="background-size:cover" background="images/12.jpg">
	<form action="order_delete.php" method="POST">
		<div>Enter ORDER_ID:<input type="text" name="ORDER_ID"><br></div>
		<br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if($a!="")
        {
            $sql1 = "SELECT * from orders where order_id='$a'";
            $result = mysqli_query($con,$sql1);

            if(mysqli_num_rows($result)>0)
            {
            $sql3="DELETE from orders where order_id='$a'";  
            mysqli_query($con,$sql3);
            echo "<b>Record with order_id = $a is deleted successfully.<b>";
            }
           else
        {
